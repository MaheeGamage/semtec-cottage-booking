...
//	        Resource hasMappingResource = extractHasMapping(ontModel);
//	        if (hasMappingResource != null) {
//	        	ServiceUtil.printResourceProperties(hasMappingResource);
//	            // Convert extracted data to JSON (for demonstration)
//	            String extractedData = extractHasMappingDataToJson(hasMappingResource);
//	            resp.getWriter().write(extractedData);
//	        } else {
//	            resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
//	            resp.getWriter().write("{\"error\": \"No valid hasMapping section found\"}");
//	        }
	    } catch (Exception e) {
	        e.printStackTrace();
    ...
    private String extractHasMappingDataToJson(Resource hasMappingResource) {
        ...
        Property bedroomCountProp = ontModel.createProperty("http://localhost:8080/SW_project/cottagebooking#bedroomCount");
